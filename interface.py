...
class Interface:
    def __init__(self, quiz: QuizBrain):
        ...
        self.quiz = quiz
        self.score_text = Label(text=f"Score: {quiz.score}",
                                bg=BG, fg="white", font=("Franklin Gothic Demi", 12))
        self.score_text.grid(column=1, row=0)
        true_icon = PhotoImage(file=path.join(directory, "art", "true.png"))
        false_icon = PhotoImage(file=path.join(directory, "art", "false.png"))
        ...
        self.question = self.canvas.create_text(
            180, 140, width=340, font=("Franklin Gothic Book", 18, "italic"))
        self.show_question()
    ...
    def show_question(self):
        ...
        if self.quiz.question_number < self.quiz.total_questions:
            ...
            self.canvas.itemconfig(self.question,
                                   text=f"Your final score is {self.quiz.score}")
    ...
    def feedback(self, result):
        if result:
            ...
            pygame.mixer.init()
            pygame.mixer.music.load(path.join(directory, "art", "correct.wav"))
            pygame.mixer.music.play()
        elif not result:
            self.canvas.config(bg="red")
            pygame.mixer.init()
            pygame.mixer.music.load(path.join(directory, "art", "wrong.wav"))
            pygame.mixer.music.play()
